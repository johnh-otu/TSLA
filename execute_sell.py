...
parser.add_argument('--shares', type=int, required=True, help='The amount of shares to be sold')
args = parser.parse_args()

shares_to_sell = args.shares
...
capital_after_fee = shares_to_sell * current_price * (1 - transaction_fee_rate)

if (num_shares_held < shares_to_sell):
    print("No shares to sell.")
else:
    portfolio["capital"] += capital_after_fee
    portfolio["shares"] -= shares_to_sell
    update_portfolio(portfolio)
    print(f"Sell: ${shares_to_sell} shares. ({capital_after_fee:.2f}) capital gained.")